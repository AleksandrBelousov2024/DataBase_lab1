...
- **Seq Scan** - последовательное чтение всей таблицы (медленно для больших таблиц)
- **Index Scan** - использование индекса для поиска (быстро)
- **Nested Loop** - вложенные циклы (медленно при больших данных)
- **Hash Join** - соединение через хеш-таблицы (быстрее для больших объемов)
- **Sort** - сортировка результатов (дорогая операция)
...
- **Seq Scan на visits** - читает всю таблицу visits (20,000 строк)
- **Hash Join** - создает хеш-таблицу для соединения
- **Фильтрация по doctor_id** - без индекса, проверяет все строки
- **Общее время: ~200 мс** - много для одного запроса

**Проблема:** Нет индекса для поиска записей по doctor_id, поэтому проверяются все 20,000 записей.
...
**Анализ результатов (до оптимизации):**
- **Seq Scan на обе таблицы** - читает все 40,000 строк (пациенты + визиты)
- **Hash Join** - хеш-соединение без индексов
- **GroupAggregate** - группировка по всем пациентам
- **Sort** - сортировка 20,000 результатов
- **Общее время: ~250 мс** - операции группировки и сортировки дорогие

**Проблема:** Нет индекса для быстрого поиска визитов по patient_id, группировка и сортировка выполняются над всеми данными.
...
**Анализ результатов (до оптимизации):**
- **Seq Scan на visits с фильтром** - фильтрация по дате без индекса
- **HashAggregate** - двойная агрегация (COUNT и COUNT DISTINCT)
- **Нет индекса по дате** - проверяет все 20,000 записей
- **Общее время: ~350 мс** - сложные агрегации на больших данных

**Проблема:** Нет индекса для поиска по дате, двойная агрегация выполняется над отфильтрованными данными.
...
- **Seq Scan на visits** - читает 20,000 строк
- **Фильтр: (doctor_id = 100)** - проверяет каждую строку
- **Время: ~180 мс** - медленно из-за полного сканирования
...
**Анализ ПОСЛЕ оптимизации (с индексом idx_visits_doctor_date):**
- **Index Scan using idx_visits_doctor_date** - использует индекс
- **Прямой доступ** к нужным записям врача 100
- **Время: ~25 мс** - в 7 раз быстрее
- **Чтение только нужных страниц** - уменьшение I/O операций

**Как индекс повлиял на SELECT:**
1. **Вместо Seq Scan → Index Scan** - прямой доступ по индексу
2. **Чтение только релевантных записей** - не вся таблица
3. **Ускорение соединений** - быстрый поиск связей
4. **Уменьшение памяти** - меньше данных в хеш-таблицах
...
**Анализ ДО оптимизации:**
- **Seq Scan на patients с фильтром LIKE** - медленный поиск по ФИО
- **Seq Scan на visits** - полное чтение визитов
- **Hash Join** - создание хеш-таблицы для соединения
- **Время: ~280 мс** - операции над всеми данными
...
**Анализ ПОСЛЕ оптимизации (с индексами idx_patients_full_name и idx_visits_patient_date):**
- **Index Scan using idx_patients_full_name** - быстрый поиск по ФИО
- **Index Scan using idx_visits_patient_date** - прямой доступ к визитам пациента
- **Ускорение LIKE запросов** - индекс поддерживает префиксный поиск
- **Время: ~45 мс** - в 6 раз быстрее
...
### 1. **Индексы кардинально меняют способ выполнения запросов**
- **До:** Seq Scan (последовательное чтение всей таблицы)
- **После:** Index Scan (прямой доступ к нужным данным)
- **Эффект:** Ускорение в 5-10 раз

### 2. **Ключевые индексы для медицинской системы:**
- **idx_visits_doctor_date** - для поиска пациентов врача
- **idx_visits_patient_date** - для истории пациента
- **idx_patients_full_name** - для быстрого поиска по ФИО
- **idx_visits_date** - для фильтрации по дате

### 3. **Улучшение конкретных операций:**
- **WHERE условия:** Индексы превращают полное сканирование в прямое чтение
- **JOIN операции:** Ускорение соединений через индексные сканы
- **GROUP BY:** Эффективная группировка через индексированные данные
- **ORDER BY:** Использование индексов для предварительной сортировки

### 4. **Показатели производительности:**
| Запрос | До оптимизации | После оптимизации | Ускорение |
|--------|----------------|-------------------|-----------|
| Поиск пациентов врача | 180 мс | 25 мс | 7.2x |
